...
from .nougat_adapter import run_nougat
...
def convert_pdf_to_txt(pdf_path: Path, txt_path: Path, engine: str = "hybrid") -> None:
    ...
    if engine in ("native", "hybrid"):
        text, meta = extract_text_with_structure(pdf_path)
        metadata.update(meta)
        metadata["engine"] = engine

    if engine == "nougat" or (engine == "hybrid" and _is_text_low_quality(text)):
        print(f"→ Using Nougat for {pdf_path.name}")

        try:
            nougat_dir = txt_path.parent / f"{txt_path.stem}.nougat"
            nougat_text = run_nougat(pdf_path, nougat_dir)

            if nougat_text.strip():
                text = nougat_text
                metadata["used_nougat"] = True
        except Exception as e:
            print(
                f"Warning: Nougat failed for {pdf_path}: {e}",
                file=sys.stderr,
            )
...
def convert_pdf_directory(source_dir: Path, output_dir: Path, refresh: bool = False) -> int:
    ...
    if source_dir == output_dir or output_dir in source_dir.parents:
        raise ValueError("Output directory must not be the same as or nested under the source directory.")
